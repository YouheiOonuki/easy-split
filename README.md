...
3. **＋／−** で人数を合わせる。名前は結果の下の「名前・係数・固定」で入力
...
- yorozu-craft.com の各ツールは同じオリジンでキャッシュ領域を共有しています。`sw.js` で古いキャッシュを消すのは `easy-split-` で始まるものだけにしてください（ほかのツールのキャッシュを消すと、そのツールがオフラインで開けなくなります）
